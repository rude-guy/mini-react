...
export function readContext<T>(consumer: FiberNode, context: ReactContext<T>) {
  if (consumer === null) {
    ...
    if (deps !== null) {
      ...
      while (contextItem !== null) {
        if (contextItem.context === context) {
          ...
          // 往上遍历
          schedulerContextWorkOnParentPath(fiber.return, wip, renderLane);
...
function schedulerContextWorkOnParentPath(
  form: FiberNode | null,
  to: FiberNode,
  renderLane: Lane
) {
